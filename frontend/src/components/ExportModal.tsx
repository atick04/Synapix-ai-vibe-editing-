...
export default function ExportModal({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(4px)' }}>
            <div className="w-full max-w-lg rounded-none border border-zinc-900 overflow-hidden font-mono" style={{ background: '#09090b' }}>
                <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-900 bg-[#08080a]">
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-none border border-zinc-800 bg-[#0c0c0e] flex items-center justify-center text-amber-500">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-zinc-550 lowercase">ffmpeg render preset controls</p>
                        </div>
                    </div>
                ...
                <div className="px-6 py-5 space-y-5">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-[9px] font-bold text-zinc-550 mb-2 uppercase tracking-widest">resolution</label>
                            <div className="flex gap-2">
                                {['720p','1080p','4k'].map(r => (
                                    <button key={r} onClick={() => setExportResolution(r)}
                                        className={`flex-1 py-1.5 border text-[11px] font-bold transition-all rounded-none cursor-pointer ${
                        ...
                        <div>
                            <label className="block text-[9px] font-bold text-zinc-550 mb-2 uppercase tracking-widest">framerate</label>
                            <div className="flex gap-2">
                                {[24,30,60].map(f => (
                    ...
                    <div>
                        <label className="block text-[9px] font-bold text-zinc-550 mb-2 uppercase tracking-widest">quality profile</label>
                        <div className="grid grid-cols-3 gap-2">
                            {[{v:'high',l:'high',s:'crf 18'},{v:'medium',l:'medium',s:'crf 23'},{v:'fast',l:'fast',s:'crf 28'}].map(q => (
                                <button key={q.v} onClick={() => setExportQuality(q.v)}
                                    className={`py-2 px-3 text-left border transition-all rounded-none cursor-pointer ${
                                        exportQuality === q.v ? 'border-amber-500/30 bg-amber-500/10' : 'border-zinc-900 bg-zinc-950/20 hover:border-zinc-800'
                                    }`}>
                                    <div className={`text-[11px] font-bold lowercase ${exportQuality === q.v ? 'text-amber-500' : 'text-zinc-400'}`}>{q.l}</div>
                                    <div className="text-[9px] text-zinc-600 mt-0.5">{q.s}</div>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-[9px] font-bold text-zinc-550 mb-2 uppercase tracking-widest">format & codec</label>
                            <div className="space-y-1.5">
                                {[{v:'mp4_h264',l:'mp4 / h.264'},{v:'mp4_h265',l:'mp4 / h.265'},{v:'webm',l:'webm / vp9'}].map(fmt => (
                                    <button key={fmt.v} onClick={() => setExportFormat(fmt.v)}
                                        className={`w-full py-1.5 px-3 text-left text-[10px] font-bold border transition-all rounded-none cursor-pointer ${
                        ...
                        <div>
                            <label className="block text-[9px] font-bold text-zinc-550 mb-2 uppercase tracking-widest">audio bitrate</label>
                            <div className="space-y-1.5">
                    ...
                    <div className="px-4 py-2 text-[10px] text-zinc-500 flex items-center gap-2 border border-zinc-900 bg-zinc-950/40">
                    ...
                    {exportDone && (
                        <div className="px-4 py-2.5 flex items-center gap-3 border border-amber-500/20 bg-amber-500/5 text-amber-500">
                            <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                            <span className="text-[11px] font-mono lowercase">export complete! ready to download.</span>
                            <a href={`${API_URL}/uploads/${id}_rendered.mp4`} download
                                className="ml-auto px-3 py-1 bg-amber-500 hover:bg-amber-600 text-black font-bold text-[10px] transition-colors rounded-none"
                            >
                                download
                            </a>
                        </div>
                ...
                <div className="px-6 py-4 border-t border-zinc-900 flex justify-end gap-3 bg-[#08080a]">
                    <button onClick={onClose} className="px-4 py-2 border border-zinc-900 hover:border-zinc-800 text-[11px] text-zinc-500 hover:text-zinc-300 transition-all rounded-none cursor-pointer">
                        cancel
                    </button>
                    <button
                        disabled={isExporting}
                        onClick={handleExport}
                        className="flex items-center gap-2 px-5 py-2 bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-black text-[11px] font-bold transition-all rounded-none cursor-pointer"
                    >
                        {isExporting ? (
                            <>
